...
SELECT c.country_name, i.Measles_coverage, i.HPV_coverage, m.Under_five_mortality_rate, m.Neonatal_mortality_rate
	FROM Countries c
		JOIN immunisation i ON c.country_ID = i.country_ID
			JOIN MortalityData m ON c.country_ID = m.country_ID
				WHERE i.Measles_coverage > 50 AND i.HPV_coverage > 50;
...
# investigate maternal mortality based on if a skilled worker attended to the mother
SELECT c.country_name, m.Maternal_mortality_ratio, m.Proportion_of_births_attended
	FROM Countries c
		JOIN MortalityData m ON c.country_ID = m.country_ID
			ORDER BY m.Maternal_mortality_ratio DESC;

# compare if life expectancy increases with an increase in workforce
SELECT c.country_name, le.Total_Life_expectancy, hwd.doctors_density, hwd.nursing_midwifery_density
	FROM Countries c
		JOIN LifeExpectancy le ON c.country_ID = le.country_ID
			JOIN healthWorkforce_density hwd ON c.country_ID = hwd.country_ID
				ORDER BY le.Total_Life_expectancy DESC;

# impact of health expenditure on disease burden
...
SELECT c.country_name, cd.HIV_prevalence, cd.TB_prevalence, he.expenditure_over_25, le.Total_Life_expectancy
	FROM Countries c
		JOIN chronic_diseases cd ON c.country_ID = cd.country_ID
			JOIN health_expenditure he ON c.country_ID = he.country_ID
				JOIN LifeExpectancy le ON c.country_ID = le.country_ID
